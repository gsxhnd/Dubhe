/// The entry is a type of change that needs to be applied. It contains two data
/// fields. While the fields are built into the model; their usage is determined
/// by the entry_type.
///
/// For normal entries, the data field should contain the data change that should
/// be applied. The context field can be used for any contextual data that might
/// be relevant to the application of the data.
///
/// For configuration changes, the data will contain the ConfChange message and
/// the context will provide anything needed to assist the configuration change.
/// The context if for the user to set and use in this case.
...
pub struct Message {
    ...
    /// example, (type=MsgAppend,index=100,log_term=5) means leader appends entries
    /// starting at index=101, and the term of entry at index 100 is 5.
    /// (type=MsgAppendResponse,reject=true,index=100,log_term=5) means follower
    /// rejects some entries from its leader as it already has an entry with term 5
    /// at index 100.
    #[prost(uint64, tag = "5")]
    pub log_term: u64,
    #[prost(uint64, tag = "6")]
...
pub struct ConfState {
    ...
    /// The voters in the outgoing config. If not empty the node is in joint
    /// consensus.
