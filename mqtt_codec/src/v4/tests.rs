...
mod tests {
    ...
    use crate::v4::{
        ConnAckPacket, ConnectPacket, ConnectReturnCode, MqttCodec, Packet,
        PubAckPacket, PubCompPacket, PubRecPacket, PubRelPacket,
        PublishPacket, QoS, SubAckPacket, SubAckReturnCode,
    ...
    fn test_pubrec_packet_encoding_decoding() {
        ...
        let pubrec_packet = Packet::PubRec(PubRecPacket { packet_id: 300 });

        codec.encode(pubrec_packet.clone(), &mut buffer).unwrap();
        let decoded_packet = codec.decode(&mut buffer).unwrap().unwrap();

        match (pubrec_packet, decoded_packet) {
    ...
    fn test_pubrel_packet_encoding_decoding() {
        ...
        let pubrel_packet = Packet::PubRel(PubRelPacket { packet_id: 300 });

        codec.encode(pubrel_packet.clone(), &mut buffer).unwrap();
        let decoded_packet = codec.decode(&mut buffer).unwrap().unwrap();

        match (pubrel_packet, decoded_packet) {
    ...
    fn test_pubcomp_packet_encoding_decoding() {
        ...
        let pubcomp_packet = Packet::PubComp(PubCompPacket { packet_id: 300 });

        codec.encode(pubcomp_packet.clone(), &mut buffer).unwrap();
        let decoded_packet = codec.decode(&mut buffer).unwrap().unwrap();

        match (pubcomp_packet, decoded_packet) {
    ...
    fn test_unsubscribe_packet_encoding_decoding() {
        ...
        codec.encode(unsubscribe_packet.clone(), &mut buffer).unwrap();
        let decoded_packet = codec.decode(&mut buffer).unwrap().unwrap();

        match (unsubscribe_packet, decoded_packet) {
    ...
    fn test_unsuback_packet_encoding_decoding() {
        ...
        let unsuback_packet = Packet::UnsubAck(UnsubAckPacket { packet_id: 5 });

        codec.encode(unsuback_packet.clone(), &mut buffer).unwrap();
        let decoded_packet = codec.decode(&mut buffer).unwrap().unwrap();

        match (unsuback_packet, decoded_packet) {
    ...
    fn test_pingreq_packet_encoding_decoding() {
        ...
        codec.encode(pingreq_packet.clone(), &mut buffer).unwrap();
        assert_eq!(buffer.len(), 2); // Fixed header only: 0xC0 0x00

        let decoded_packet = codec.decode(&mut buffer).unwrap().unwrap();

        match (pingreq_packet, decoded_packet) {
    ...
    fn test_pingresp_packet_encoding_decoding() {
        ...
        codec.encode(pingresp_packet.clone(), &mut buffer).unwrap();
        assert_eq!(buffer.len(), 2); // Fixed header only: 0xD0 0x00

        let decoded_packet = codec.decode(&mut buffer).unwrap().unwrap();

        match (pingresp_packet, decoded_packet) {
    ...
    fn test_disconnect_packet_encoding_decoding() {
        ...
        codec.encode(disconnect_packet.clone(), &mut buffer).unwrap();
        assert_eq!(buffer.len(), 2); // Fixed header only: 0xE0 0x00

        let decoded_packet = codec.decode(&mut buffer).unwrap().unwrap();

        match (disconnect_packet, decoded_packet) {
